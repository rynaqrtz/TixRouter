...
name: rynarouter-cli
description: |
    Comprehensive development skill for the RYNArouter CLI (@rynarouter/cli / apps/cli). Use whenever working on, modifying, debugging, or reviewing: Commander.js command orchestration, rynarouter CLI commands (setup, init, link, unlink, status, doctor, sync, env, run, migrate), adapter integrations (Claude Code ~/.claude.json, OpenCode ~/.opencode.json), backup/restore snapshotting (~/.rynarouter/backups/), shell environment variable exports (bash, zsh, fish, powershell), interactive terminal prompts (@clack/prompts), or dry-run safety modes.
---

# ⚡ RYNArouter — CLI Skill

Development guide for `apps/cli` (`@rynarouter/cli`).
