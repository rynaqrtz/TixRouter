    ...
    return (
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 font-mono">
            ...
            >
                ...
                />
                <StatCard
                    label="Saved (Free Tier)"
                    value={stats?.savings?.savedLabel ?? "$0.00"}
                    tooltip={
                        stats?.savings
                            ? `${stats.savings.freeRequests.toLocaleString()} free-tier requests · ${stats.savings.freeTokens.toLocaleString()} tokens · list-price value ${stats?.costLabel}`
                            : undefined
                    }
                    detail={
                        stats?.savings?.freeRequests
                            ? `${formatCompactNumber(stats.savings.freeRequests)} free requests · ${formatCompactNumber(stats.savings.freeTokens)} tokens`
                            : "No free-tier traffic yet"
                    }
                    icon={PiggyBank}
                />
                <StatCard
